...
for (const language of ['en', 'es']) {
  ...
  if (content.results?.items?.length !== 4) failures.push(`${language}: expected 4 result cards`);
...
if (failures.length) {
  console.error('Portfolio v3.12 verification failed:');
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log('Portfolio v3.12 verification: OK');
